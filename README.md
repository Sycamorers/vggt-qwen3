...
Once you have:
- Completed Stage 1 (or are using the provided Stage 1 checkpoint shards), and
...
### 11.2 Single-GPU ARKit inference (Stage 1 weights)
...
python -m src.inference.arkit_inference \
  ...
  --max_new_tokens 256 \
  --device cuda:0 \
  --output_jsonl ckpts/arkit_infer/arkit_predictions_stage1_en.jsonl
```
...
- The script prints per-sample logs and writes all results to `ckpts/arkit_infer/arkit_predictions_stage1_en.jsonl`.
