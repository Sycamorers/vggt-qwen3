...
This guide aligns with the current repository state: Qwen3-4B + VGGT, Stage 2 (3D QA) configured via `configs/stage2_3d.yaml`, single-view ScanQA/SQA3D shards in `data/processed/`, and the hardened launcher `train_fixed.sh`. Use this as a companion to the main README.
...
- `src/dataio/dataset_builder.py` reads `.json` arrays or `.jsonl` lines and truncates to `num_views` (8 in Stage 2; single-view is accepted).
...
- **Active config:** `configs/stage2_3d.yaml`
...
- Configs: `configs/stage2_3d.yaml`, `configs/perceiver_small.yaml`, `configs/deepspeed_zero3.json`
