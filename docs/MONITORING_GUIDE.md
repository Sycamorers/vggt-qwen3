...
Applies to the current Stage 2 setup (`train_fixed.sh` + `configs/stage2_3d.yaml`). All commands assume repo root as CWD.
...
|------|---------|---------|
| TensorBoard | `tensorboard --logdir ckpts/stage2_3d/logs --port 6006 --bind_all` | Full visualization of loss/LR/speed |
| CLI Monitor (snapshot) | `python scripts/monitor_training.py --logdir ckpts/stage2_3d/logs/roomplan` | One-time metrics, ASCII plot |
| CLI Monitor (watch) | `python scripts/monitor_training.py --logdir ckpts/stage2_3d/logs/roomplan --watch --interval 30` | Auto-refreshing metrics |
...
- Disk space: `du -sh ckpts/stage2_3d` and `df -h .`

---

## 5) Troubleshooting Monitoring
- **No events in TensorBoard:** Ensure `ckpts/stage2_3d/logs/roomplan` exists; training must reach first `log_every_steps`.
- **Monitor script shows 0 steps:** Confirm the `--logdir` matches the `project_dir` used by `train_fixed.sh` (defaults under `ckpts/stage2_3d/logs/roomplan`).
