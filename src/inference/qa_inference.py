...
def main() -> None:
    ...
    # By default, iterate all records deterministically.
    ...
    print(
        f"Loaded {total_records} records from glob '{args.glob}'. "
        f"Preparing {len(selected_indices)} records for inference."
    )
    ...
    if len(results) != len(samples):
        raise RuntimeError(
            f"Sanity check failed: prepared {len(samples)} samples but "
            f"received {len(results)} predictions."
        )

    print(
        f"✅ Finished inference: loaded {total_records} records, "
        f"ran inference on {len(samples)} records, "
        f"wrote {len(results)} predictions to {output_path or '<stdout only>'}."
    )
