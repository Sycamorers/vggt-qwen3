...
def build_dataloader(cfg: Dict, tokenizer) -> DataLoader:
    ...
    for name, glob_path in cfg["datasets"].items():
        ds_cfg = DatasetConfig(path_glob=glob_path, num_views=cfg["num_views"], image_size=cfg["image_size"], task=name)
    ...
    loader = DataLoader(multi, batch_size=cfg["train_batch_size"], shuffle=True, collate_fn=collator)
    return loader


def build_model(cfg: Dict) -> VGGTQwen3VLM:
    proj_cfg = load_yaml(cfg["model"]["projector"]) if isinstance(cfg["model"]["projector"], str) else cfg["model"]["projector"]
...
def main() -> None:
    ...
    tokenizer = build_tokenizer(stage_cfg["model"]["name_or_path"], stage_cfg["model"].get("tokenizer_path"))
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": projector_params, "lr": train_cfg.get("proj_lr", train_cfg["lr"])},
        ],
        weight_decay=train_cfg["weight_decay"],
    )
    num_warmup_steps = int(train_cfg["warmup_ratio"] * max_steps)
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, max_steps)
    ...
    for batch in dataloader:
        ...
        if accelerator.is_main_process and step % train_cfg["log_every_steps"] == 0:
            ...
            eta_seconds = (max_steps - step - 1) / steps_per_sec if steps_per_sec > 0 else 0
            eta_hours = eta_seconds / 3600
            
            # Get learning rates
            base_lr = optimizer.param_groups[0]['lr']
            proj_lr = optimizer.param_groups[1]['lr'] if len(optimizer.param_groups) > 1 else base_lr
        ...
        if step >= max_steps:
            break
        if accelerator.is_main_process and train_cfg["save_every_steps"] and step % train_cfg["save_every_steps"] == 0:
